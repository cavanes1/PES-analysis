program main
  ...
  character(3),     dimension(:), allocatable :: aname
...
contains
  ...
  !*
  subroutine calcHess(natom,cgeom,nstate,istate,stepsize,hessian,centerd,skip)
    implicit none
    integer, intent(in)          :: natom, nstate,istate
    logical, intent(in),optional :: skip(natom)
    logical, intent(in)          :: centerd 
    double precision,intent(in)  :: stepsize
    double precision,intent(in)  :: cgeom(3*natom)
    double precision,intent(out) :: hessian(3*natom,3*natom)
    double precision   ::  mdif
    
    integer           ::   i,  j
    logical           ::   skipdisp(natom*3)
    double precision  ::  dispgeom(3*natom), dgrd(3*natom),gref(3*natom)
    real*8            ::  e(nstate)
    real*8            ::  cg(3*natom,nstate,nstate)
    real*8            ::  h(nstate,nstate)
    real*8            ::  dcg(3*natom,nstate,nstate)
    real*8            ::  ckl(nstate,nstate)
    ...
    if(.not.centerd)then
      call Evaluate(cgeom,e,cg,h,dcg,ckl)
    ...
    do i=1,3*natom
      ...
      call Evaluate(dispgeom,e,cg,h,dcg,ckl)
